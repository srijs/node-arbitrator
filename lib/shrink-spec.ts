...
describe('Shrink', () => {
  ...
  describe('removes', () => {
    it('permutes a list by removing k consecutive elements from it', () => {
      expect(Array.from(removes(2, [1,2,3,4,5,6])))
  ...
  describe('halves', () => {
    it('produces a list containing the results of halving a number over and over again', () => {
      expect(Array.from(halves(30))).toEqual([30,15,7,3,1]);
      expect(Array.from(halves(128))).toEqual([128,64,32,16,8,4,2,1]);
      expect(Array.from(halves(-10))).toEqual([-10,-5,-2,-1]);
    });
  });
